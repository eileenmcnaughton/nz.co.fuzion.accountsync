...
spl_autoload_register('_accountsync_civix_class_loader', TRUE, TRUE);

function _accountsync_civix_class_loader($class) {
  if ($class === 'CRM_Accountsync_DAO_Base') {
    if (version_compare(CRM_Utils_System::version(), '5.74.beta', '>=')) {
      class_alias('CRM_Core_DAO_Base', 'CRM_Accountsync_DAO_Base');
      // ^^ Materialize concrete names -- encourage IDE's to pick up on this association.
    }
    else {
      $realClass = 'CiviMix\\Schema\\Accountsync\\DAO';
  ...
  if (strpos($class, 'CiviMix\\Schema\\Accountsync\\') === 0) {
